@foreach ($plugins as $plugin)
    @switch($plugin)
        ...
        @case('dropzone')
            ...
            @push('styles')
                <link rel="stylesheet" type="text/css"
                    href="{{ asset('back/app-assets/vendors/css/file-uploaders/dropzone.min.css') }}">
                <link rel="stylesheet" type="text/css"
                    href="{{ asset('back/app-assets/css-rtl/plugins/file-uploaders/dropzone.css') }}">
            @endpush
        @break
        ...
        @case('persian-datepicker')
            ...
            @push('styles')
                <link rel="stylesheet" type="text/css"
                    href="{{ asset('back/app-assets/plugins/persian-date/persian-datepicker.min.css') }}">
            @endpush
        @break
        ...
        @case('apexcharts')
            @push('plugin-scripts')
                <script src="{{ asset('back/app-assets/vendors/js/charts/apexcharts.min.js') }}?v=51"></script>
            @endpush
        ...
        @case('combo-tree')
            @push('plugin-scripts')
                <script src="{{ asset('back/app-assets/plugins/combo-tree/combo-tree.js') }}?v=51"></script>
            @endpush

            @push('styles')
                <link rel="stylesheet" href="{{ asset('back/app-assets/plugins/combo-tree/combo-tree.css') }}?v=51">
            @endpush
        @break
