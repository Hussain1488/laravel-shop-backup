                                            ...
                                            <fieldset class="form-group">
                                                <select class="form-control" name="status" id="filter-status">
                                                    ...
                                                    <option value="pending"
                                                        {{ request('status') == 'pending' ? 'selected' : '' }}>منتظر تایید
                                                    </option>
                                                    <option value="accepted"
                                                        {{ request('status') == 'accepted' ? 'selected' : '' }}>تایید شده
                                                    </option>
                                                    <option value="rejected"
                                                        {{ request('status') == 'rejected' ? 'selected' : '' }}>تایید نشده
                                                    </option>
                                                </select>
                                            </fieldset>
                                        </div>
                                        <div class="col-12 col-sm-6 col-lg-3">
                                            ...
                                            <fieldset class="form-group">
                                                <select class="form-control" name="ordering" id="filter-ordering">
                                                    <option value="latest"
                                                        {{ request('ordering') == 'latest' ? 'selected' : '' }}>جدیدترین
                                                    </option>
                                                    <option value="oldest"
                                                        {{ request('ordering') == 'oldest' ? 'selected' : '' }}>قدیمی ترین
                                                    </option>
                                                </select>
                                            </fieldset>
                                        </div>
                                            ...
                                            <tbody>
                                                @foreach ($reviews as $review)
                                                    <tr id="review-{{ $review->id }}-tr">
                                                        ...
                                                        <td>{{ $review->user ? $review->user->fullname : $review->name }}
                                                        </td>
                                                        <td style="max-width: 300px">
                                                            {{ short_content($review->body, 20, false) }}</td>
                                                        <td style="max-width: 200px">
                                                            <span
                                                                class="d-inline-block">{{ $review->shop->nameofstore }}</span>
                                                            {{-- <a href="{{ Route::has('front.products.show') ? route('front.products.show', ['product' => $review->product]) : '' }}" --}}
                                                        ...
                                                        <td class="text-center text-nowrap">
                                                            <button type="button"
                                                                data-action="{{ route('admin.shop_more.show', [$review->id]) }}"
                                                                data-review="{{ $review->id }}"
                                                                class="btn btn-success waves-effect waves-light show-review">مشاهده</button>
                                                            <button data-review="{{ $review->id }}"
                                                                data-action="{{ route('admin.shop_more.delete', [$review->id]) }}"
                                                                type="button"
    ...
    <div class="modal fade" id="show-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div id="shop-review-detail" class="modal-body"></div>
                <div class="modal-footer">
                    <button id="shop-review-form-submit-btn" type="button"
                        class="btn btn-outline-success">ذخیره</button>
...
@push('scripts')
    <script src="{{ asset('back/assets/js/pages/cooperationsales/reviewindex.js') }}?v=51"></script>
@endpush
