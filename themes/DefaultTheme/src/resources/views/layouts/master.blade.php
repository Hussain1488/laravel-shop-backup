...
<head>
    ...
    <meta name="google-site-verification" content="qXbe6iGLaeQr3T_pkCsFIy3ZU-pTPIRIzBcNeQUlRcg" />
    ...
    @if (config('app.debug'))
        ...
        <!-- Main CSS File -->
        <link rel="stylesheet" href="{{ theme_asset('css/styles.css') }}?v=51">
        <link rel="stylesheet" href="{{ theme_asset('css/custom.css') }}?v=51">
        <link rel="stylesheet" href="{{ theme_asset('css/main.css') }}?v=51">
        <!-- endinject -->
    @else
        <!-- All Css Files -->
        <link rel="stylesheet" href="{{ theme_asset('css/all.css', config('front.mainfest_path')) }}?v=51">
    @endif

    @if ($current_local['direction'] == 'ltr')
        <link rel="stylesheet" href="{{ theme_asset('css/ltr.css') }}?v=51">
    @endif
...
<body>
                            ...
                            <div class="logo-area float-right">
                                <a href="{{ route('front.index') }}">
                                    <img data-src="{{ option('info_logo', theme_asset('img/logo.png')) }}"
                                        alt="{{ option('info_site_title', 'خانه اقساط') }}" style="width: 45px">
                                </a>
                            </div>
                            <div class="websiteDailyCodeGeneratorButton d-flex align-items-center">
                                <input type="image" src="{{ theme_asset('img/gift.gif') }}" class="">
                                <span>قرعه کشی</span>
                            </div>
    ...
    @if (config('app.debug'))
        ...
        <!-- Main JS File -->
        <script src="{{ theme_asset('js/main.js') }}?v=51"></script>
        <script src="{{ theme_asset('js/custom.js') }}"></script>
        <script src="{{ theme_asset('js/scripts.js') }}?v=51"></script>
    @else
        <!-- All JS Files -->
        <script src="{{ theme_asset('js/all.js', config('front.mainfest_path')) }}?v=51"></script>
    @endif
