        ...
        <form id="products-filter-form" action="{{ route('front.products.category-products', ['category' => $category]) }}">
            ...
            <div class="col-12 filter-product mb-3">
                <div class="accordion" id="accordionExample">
                    ...
                    @php
                        $fitting->products_id = $category->allPublishedProducts()->pluck('id');
                    @endphp
                    ...
                    @foreach ($category->getFilter()->related()->orderBy('ordering')->get() as $filter)
                        ...
                        @switch($filter->filterable_type)
                            @case('App\Models\Specification')
                                ...
                                @php
                                    $spec_values = \DB::table('product_specification')->whereIn('product_id', $fitting->products_id)->where('specification_id', $filter->filterable_id)->get()->unique('value')->pluck('value')->toArray();
                            ...
                            @case('App\Models\AttributeGroup')
                                ...
                                @php
                                    $prices = \DB::table('prices')->whereIn('product_id', $fitting->products_id)->pluck('id');
