...
<div class="col-lg-8 mt-3 col-md-12 pb-5 product-info-block">
    <div class="product-info dt-sl">
        <div class="product-title">
            <h1>{{ $fitting->product->title }}</h1>
            <h3 class="mb-1">{{ $fitting->product->title_en }}</h3>
        </div>
        <div class="row pt-2">
            <div class="col-md-7 col-lg-7">
                <hr class="border-product-title">
                <div class="row mt-2">
                    @if ($fitting->product->rating)
                        <div class="col-12 d-flex">
                            <div class="d-flex">
                                <i class="mdi mdi mdi-star text-warning mx-0"></i>
                                <p class="mx-1 mb-2">{{ $fitting->product->rating }}</p>
                                <p class="mx-1 mb-2 text-muted">({{ $fitting->product->reviews_count }})</p>
                            </div>
                            <div class="d-flex">
                                <p class="text-primary font-weight-bold mx-2 mb-2">
                                    {{ trans('front::messages.products.review') }} {{ $fitting->product->reviews_count }}</p>
                                <p class="text-primary font-weight-bold mx-2 mb-2">
                                    {{ trans('front::messages.products.comment-count') }}
                                    {{ $fitting->product->comments()->accepted()->count() }}</p>
                            </div>
                        </div>
                    @endif
                    @if ($fitting->product->suggestionCount())
                        <div class="col-12 d-flex">
                            <i class="mdi mdi mdi-thumb-up-outline text-success mx-0"></i>
                            <p class="text-muted commodity mx-2">
                                <span>{{ $fitting->product->suggestionPercent() }}%</span>({{ $fitting->product->suggestionCount() }}) نفر
                                از خریداران این کالا را پیشنهاد کردن
                            </p>
                        </div>
                    @endif
                </div>
                @if ($fitting->product->short_description)
                    <p class="little-des pt-0 mt-0">{!! nl2br($fitting->product->short_description) !!}</p>
                @endif
                @php
                    $specialSpecifications = $fitting->product->specialSpecifications();
                @endphp
                ...
                @if ($fitting->product->brand)
                    <div class="d-block mb-2">
                        ...
                        <a href="{{ route('front.brands.show', ['brand' => $fitting->product->brand]) }}"
                            class="link--with-border-bottom">{{ $fitting->product->brand->name }}</a>
                    </div>
                @endif
                @if ($fitting->product->sizeType)
            ...
            <div class="col-xl-4 col-md-5 col-sm-8 mx-sm-auto mx-0">
                ...
                @if ($fitting->product->isPhysical())
                    <div class="card box-card px-3 pb-3 pt-0 testing-container">
                        <div class="box-border"></div>
                        @if ($fitting->product->getPrices->count())
                            @php
                                $prev_attribute = null;
                                $groups = null;
                                $attributes_id = [];
                            ...
                            @foreach ($attributeGroups as $attributeGroup)
                                @if ($fitting->product->get_attributes($attributeGroup, $prev_attribute, $groups, $attributes_id))
                                    @php
                                        $checked = false;
                                        $group_checked = false;
                                    ...
                                    <div
                                        ...
                                        <ul class="product-variants float-right ml-3">
                                            @foreach ($fitting->product->get_attributes($attributeGroup, $prev_attribute, $groups, $attributes_id) as $attribute)
                                                @php
                                                    if ($group_loop != 0 && count($prev_selected_attr)) {
                                                        $has_stock = $fitting->product->hasAttributeStock(
                                                            $attribute,
                                                            $prev_selected_attr,
                                                        );
                                                    } else {
                                                        $has_stock = $fitting->product->hasAttributeStock($attribute);
                                                    }
                                                ...
                                                <li class="ui-variant product-attribute {{ $has_stock ? '' : 'unavailable' }}"
                                                    ...
                                                    <label
                                                        ...
                                                        @endif
                                                        <input data-product="{{ $fitting->product->slug }}" type="radio"
                                                            value="{{ $attribute->id }}"
                                                        ...
                                                        <div class="ui-variant--check">
                                                            <span
                                                                {{ $attributeGroup->type != 'color' ? 'product-warranty-span' : '' }}>{{ $attributeGroup->type != 'color' ? $attribute->name : '' }}</span>
                                                        </div>
                                                    </label>
                                                </li>
                                            @endforeach
                            ...
                            @endforeach
                            @php
                                $selected_price = $fitting->product->getPriceWithAttributes($attributes_id);
                            @endphp
                        @endif
                        <div class="dt-sl box-Price-number box-margin">
                            @if ($fitting->product->addableToCart())
                                <div class="mb-2 d-flex ">
                                    <span class="flex-grow-1 number">{{ $fitting->product->getUnit() }}</span>
