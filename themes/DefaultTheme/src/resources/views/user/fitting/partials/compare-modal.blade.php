        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="compare-products-form" action="{{ route('front.products.similar-compare') }}">
                    @csrf
                    @foreach ($fitting->products as $fitting->product)
                        <input type="hidden" name="products[]" value="{{ $fitting->product->id }}">
                    @endforeach
                ...
                <div class="row products-row">
                    @if ($similar_products->count())
                        @foreach ($similar_products as $fitting->product)
                            <div class="col-md-3 mb-3">
                                <div class="compare-product">
                                <a href="{{ $current_url . '/' . $fitting->product->id }}">
                                        <img class="w-100" data-src="{{ $fitting->product->image }}" alt="{{ $fitting->product->title }}">
                                        <p class="product-title">{{ $fitting->product->title }}</p>
                                    </a>
                                </div>
                            </div>
                        @endforeach
