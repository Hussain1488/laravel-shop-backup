    ...
    @case('stock')
        <div class="col-12 my-3">
            <div class="parent-switcher">
                ...
                <label class="label-switcher" for="switcher-stock">
                    {{ trans('front::messages.categories.only-available-goods') }}</label>
            </div>
        </div>
    @break
    ...
    @case('brand')
        ...
        @if ($brands->count())
            <div class="card">
                ...
                <div id="collapse-{{ $loop->index }}" class="collapse {{ request('filters.' . $filter->id) ? 'show' : '' }}"
                    aria-labelledby="heading-{{ $loop->index }}">
                    <div class="card-body">
                        @foreach ($brands as $brand)
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input"
                                    name="filters[{{ $filter->id }}][{{ $brand->id }}]"
                                    {{ request('filters.' . $filter->id . '.' . $brand->id) ? 'checked' : '' }}
                ...
                <div id="collapse-{{ $loop->index }}" class="collapse {{ request('filters.' . $filter->id) ? 'show' : '' }}"
                    ...
                    <div class="card-body">
                        @foreach ($category->childrenCategories as $chidCategory)
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input"
                                    name="filters[{{ $filter->id }}][{{ $chidCategory->id }}]"
                                    {{ request('filters.' . $filter->id . '.' . $chidCategory->id) ? 'checked' : '' }}
                                    id="customCheck-{{ $loop->parent->index }}-{{ $loop->index }}">
                                <label class="custom-control-label"
            ...
            <div class="parent-switcher mb-3">
                <label class="ui-statusswitcher">
                    <input type="checkbox" id="switcher-price" class="price_filter" name="price_filter"
                        {{ request('price_filter') ? 'checked' : '' }}>
                    <span class="ui-statusswitcher-slider">
                        <span class="ui-statusswitcher-slider-toggle"></span>
                    </span>
                </label>
                <label class="label-switcher" for="switcher-price">{{ trans('front::messages.categories.filter-by-price') }}
                </label>
            </div>
            <div class="mt-2 mb-2">
