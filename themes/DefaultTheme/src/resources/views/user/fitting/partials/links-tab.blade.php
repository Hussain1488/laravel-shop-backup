<div class="ah-tab-content dt-sl">
    ...
    <div class="description-product dt-sl mt-3 mb-3">
        <div class="container-fluid">
            @foreach ($fitting->product->prices()->whereHas('file')->orderBy('prices.ordering')->get() as $price)
                <div class="row files mb-2">
                    ...
                    <div class="col-md-4 col-6">
                        ...
                        @if ($price->isDownloadable())
                            ...
                            @else
                                <a href="{{ route('login', ['redirect' => route('front.products.show', ['product' => $fitting->product])]) }}" class="btn btn-success">
                                    {{ trans('front::messages.products.login-and-download') }}
                                </a>
                            @endif

                        @else
                            <button data-price_id="{{ $price->id }}" data-action="{{ route('front.cart.store', ['product' => $fitting->product]) }}" data-product="{{ $fitting->product->slug }}" type="button" class="btn btn-primary add-to-cart">
