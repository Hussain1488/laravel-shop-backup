...
@section('content')
                ...
                <div class="breadcrumb dt-sl">
                    <nav>
                        ...
                        @if ($fitting->product->category)
                            @foreach ($fitting->product->category->parents() as $parent)
                                <a
                                    href="{{ route('front.products.category', ['category' => $parent]) }}">{{ $parent->title }}</a>
                            @endforeach

                            <a
                                href="{{ route('front.products.category', ['category' => $fitting->product->category]) }}">{{ $fitting->product->category->title }}</a>
                        @endif
                        <span>{{ $fitting->product->title }}</span>
                    </nav>
                </div>
            </div>
            <!-- End title - breadcrumb -->
            {{-- <a href="{{ route('front.products.imageSwap') }}">کلیک کنید!</a> --}}
            ...
            <div class="dt-sn mb-3 dt-sl">
                <div class="row">
                    ...
                    <div class="col-lg-4 col-md-12 ps-relative">
                        ...
                        @elseif($fitting->product->isSpecial())
                            <div class="product-timeout position-relative pt-5 mb-4">
                                ...
                                @if ($fitting->product->special_end_date)
                                    <div id="product-special-end-date" class="countdown-timer mt-4" countdown
                                        data-date="{{ $fitting->product->special_end_date->format('D M d Y H:i:s O') }}">
                        ...
                        <ul class="gallery-options {{ $fitting->product->isSpecial() ? 'special' : '' }}">
                            @if (auth()->check())
                                @php
                                    $favorite_product = auth()
                                        ->user()
                                        ->favorites()
                                        ->where('product_id', $fitting->product->id)
                                        ->first();
                                @endphp
                                <li>
                                    <button id="add-to-favorites" data-action="{{ route('front.favorites.store') }}"
                                        data-product="{{ $fitting->product->id }}"
                                        class="add-favorites {{ $favorite_product ? 'favorites' : '' }}"><i
                                            class="mdi mdi-heart"></i></button>
                                    @if ($favorite_product)
                                        <span class="tooltip-option">
                                            {{ trans('front::messages.products.remove-from-favorites') }} </span>
                                    @else
                                        <span
                            ...
                            @if ($similar_products_count)
                                <a href="{{ route('front.products.compare', ['product1' => $fitting->product->id]) }}">
                                    <li>
                            ...
                            @can('products.update')
                                <li>
                                    <a href="{{ route('admin.products.edit', ['product' => $fitting->product]) }}"
                                        target="_blank">
                        ...
                        @if ($fitting->product->gallery()->count())
                            <div class="product-gallery mt-3">
                                <div class="product-carousel owl-carousel">
                                    <div class="item">
                                        @if (Auth::check() && Auth::user()->id == $fitting->user_id)
                                            <span class="fitting-image-download badge badge-pill badge-succes mt-2"
                                                data-action="{{ route('front.fitting.downloadImage', ['VirtualFitting' => $fitting]) }}">دانلود<i
                                                    class="mdi mdi-download "></i>

                                            </span>
                                        @endif
                                        <a class="gallery-item mt-3" href="{{ asset($fitting->photo) }}"
                                            data-fancybox="gallery" data-owl="one{{ 0 }}">
                                            <img src="{{ theme_asset('images/600-600.png') }}"
                                                data-src="{{ asset($fitting->photo) }}" class="not_downloadable_img"
                                                alt="{{ $fitting->product->title }}">
                                        </a>
                                    </div>
                                    @foreach ($fitting->product->gallery()->orderBy('ordering')->get() as $item)
                                        <div class="item">
                                ...
                                <ul
                                    ...
                                    <li class="active">
                                        <a href="#one{{ 0 }}" class="owl-thumbnail"
                                            data-slide="{{ 0 }}">
                                            <img src="{{ theme_asset('images/600-600.png') }}" class="not_downloadable_img"
                                                data-src="{{ asset($fitting->photo) }}"
                                                alt="{{ $fitting->product->title }}">
                                        </a>
                                    </li>
                                    @foreach ($fitting->product->gallery()->orderBy('ordering')->get() as $item)
                    ...
                    @include('front::user.fitting.partials.product-info')
                </div>
            </div>
                ...
                <section class="tabs-product-info mb-3 dt-sl">
                    <div class="ah-tab-wrapper dt-sl">
                        <div class="ah-tab dt-sl">
                            @if ($fitting->product->isDownload())
                                <a class="ah-tab-item" href="javascript:void(0)"><i
                                        class="mdi mdi-download"></i>{{ trans('front::messages.products.product-files') }}</a>
                            @endif

                            @if ($fitting->product->description)
                                <a class="ah-tab-item" href="javascript:void(0)"><i
                                        class="mdi mdi-glasses"></i>{{ trans('front::messages.products.general-specifications') }}</a>
                            @endif

                            @if ($fitting->product->specificationGroups()->count())
                                <a class="ah-tab-item" href="javascript:void(0)"><i
                                        class="mdi mdi-format-list-checks"></i>{{ trans('front::messages.products.technical-specifications') }}</a>
                            @endif
                            ...
                            <a class="ah-tab-item" href="javascript:void(0)"><i
                                    class="mdi mdi-comment-question-outline"></i>{{ trans('front::messages.products.comments-questions-answers') }}</a>
                        </div>
                    </div>
                    <div class="ah-tab-content-wrapper product-info px-4 dt-sl">
                        @if ($fitting->product->isDownload())
                            @include('front::user.fitting.partials.links-tab')
                        @endif
                        ...
                        @if ($fitting->product->description)
                            <div class="ah-tab-content dt-sl">
                                ...
                                <div class="description-product dt-sl mt-3 mb-3">
                                    <div class="container">
                                        <p>{!! $fitting->product->description !!}</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($fitting->product->specificationGroups()->count())
                            <div class="ah-tab-content params dt-sl">
                                ...
                                @foreach ($fitting->product->specificationGroups->unique() as $group)
                                    <section>
                                        <h3 class="params-title">{{ $group->name }}</h3>
                                        <ul class="params-list">
                                            @foreach ($fitting->product->specifications()->where('specification_group_id', $group->id)->get() as $specification)
                                                <li>
                        ...
                        @include('front::user.fitting.partials.reviews')

                        <div class="ah-tab-content dt-sl">
                            @include('front::components.comments', [
                                'model' => $fitting->product,
                                'route_link' => route('front.product.comments', ['product' => $fitting->product]),
                                'message' => trans('front::messages.products.no-comments'),
                            ])
                        </div>
                    </div>
                </section>
                ...
                <div class="modal-content">
                    <div class="modal-header pb-0">
                        <h5 class="modal-title" id="price-changes-modal-label">
                            {{ trans('front::messages.products.sales-price-chart') }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body chart-area">
                        <strong class="text-muted">{{ $fitting->product->title }}</strong>
                        ...
                        <ul class="chart-prices-label">
                            @foreach ($fitting->product->prices()->orderBy('stock', 'desc')->get() as $chart_price)
                                @php
                                ...
                                <li>
                                    <label
                                        ...
                                        <span>{{ $label != '' ? $label : $fitting->product->title }}</span>
                                    </label>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="modal-body">
                        ...
                        <ul class="share-product">
                            ...
                            <a target="_blank" class="telegram"
                                href="https://t.me/share/url?url={{ url('fitting/' . $fitting->id) }}">
                                <li class="custom-mdi mdi mdi-telegram"></li>
                            </a>

                            <a target="_blank" class="whatsapp"
                                href="https://api.whatsapp.com/send?text={{ url('fitting/' . $fitting->id) }}">
                                <li class="custom-mdi mdi mdi-whatsapp"></li>
                            </a>
                            <a target="_blank" class="eitaa"
                                href="https://eitaa.com/share/url?url={{ url('fitting/' . $fitting->id) }}">
                                <img class="w-50" style="color:orange" src="{{ theme_asset('img/eitaa.svg') }}"
                                    alt="">
                            </a>
                            <a target="_blank" class="linkedin"
                                href="https://www.linkedin.com/sharing/share-offsite/?url= {{ url('fitting/' . $fitting->id) }}">
                        ...
                        <div class="filed-link dir-ltr copy-text">
                            ...
                            <input id="shareLink" type="text" disabled value="{{ url('fitting/' . $fitting->id) }}"
                                readonly="">
    ...
    @if (auth()->check())
        @include('front::user.fitting.partials.add-review')
    @endif
    <!-- product review add modal -->

    @include('front::user.fitting.partials.sizes-modal')
    @include('front::user.fitting.partials.product-info-mobile')
@endsection
...
@push('scripts')
    ...
    <script src="{{ theme_asset('js/pages/products/show.js') }}?v=51"></script>
