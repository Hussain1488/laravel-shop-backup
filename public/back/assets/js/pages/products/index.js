...
var product_datatable = (function () {
    ...
    var options = {
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (row) {
                    return `
                        <input type="text" class="form-control parent-id-input" 
                            data-id="${row.id}" 
                            value="${row.parent_id || ''}" 
    ...
    var initDatatable = function () {
        ...
        // اضافه کردن رویداد تغییر برای input ها
        $(document).on('change', '.parent-id-input', function () {
            
            var productId = $(this).data('id');
            var parentId = $(this).val();
            var updateParentIdRoute = $('meta[name="update-parent-id-route"]').attr('content');

            // ارسال داده به سرور
            $.ajax({
                url: updateParentIdRoute, // استفاده از نام مسیر
                method: 'POST',
                data: {
                    id: productId,
                    parent_id: parentId,
                ...
                success: function (response) {
                    toastr.success('Parent ID updated successfully.');
                },
                error: function (xhr) {
                    toastr.error('Error updating Parent ID.');
                }
            });
        });
    };
